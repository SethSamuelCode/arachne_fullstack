...
Each generated project includes a CLI tool named `arachne_fullstack`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run arachne_fullstack server run --reload     # Start dev server
uv run arachne_fullstack server routes           # Show all routes
# Database commands
uv run arachne_fullstack db migrate -m "message" # Create migration
uv run arachne_fullstack db upgrade              # Apply migrations
uv run arachne_fullstack db downgrade            # Rollback migration
# User commands
uv run arachne_fullstack user create-admin       # Create admin user
uv run arachne_fullstack user create             # Create regular user
uv run arachne_fullstack user list               # List all users
```
...
# Server
uv run arachne_fullstack server run --reload
uv run arachne_fullstack server routes

# Database (Alembic wrapper)
uv run arachne_fullstack db init
uv run arachne_fullstack db migrate -m "Add users"
uv run arachne_fullstack db upgrade

# Users
uv run arachne_fullstack user create-admin       # Create admin (interactive)
uv run arachne_fullstack user create             # Create user (interactive)
uv run arachne_fullstack user list               # List all users
```
...
```bash
uv run arachne_fullstack cmd seed --count 100
uv run arachne_fullstack cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend arachne_fullstack server run --reload` |
| Start prod server | `uv run --directory backend arachne_fullstack server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend arachne_fullstack server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend arachne_fullstack db migrate -m "message"` |
| Apply migrations | `uv run --directory backend arachne_fullstack db upgrade` |
| Rollback migration | `uv run --directory backend arachne_fullstack db downgrade` |
| Show current | `uv run --directory backend arachne_fullstack db current` |
| Show history | `uv run --directory backend arachne_fullstack db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend arachne_fullstack user create-admin` |
| Create user | `uv run --directory backend arachne_fullstack user create` |
| List users | `uv run --directory backend arachne_fullstack user list` |

### Celery

| Task | Command |
|------|---------|
| Start worker | `uv run --directory backend arachne_fullstack celery worker` |
| Start beat | `uv run --directory backend arachne_fullstack celery beat` |
| Start flower | `uv run --directory backend arachne_fullstack celery flower` |
