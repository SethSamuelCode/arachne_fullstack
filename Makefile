...
db-init: docker-db
	...
	@sleep 3
	uv run --directory backend arachne_fullstack db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend arachne_fullstack db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend arachne_fullstack db upgrade

db-downgrade:
	uv run --directory backend arachne_fullstack db downgrade

db-current:
	uv run --directory backend arachne_fullstack db current

db-history:
	uv run --directory backend arachne_fullstack db history

# === Server ===
run:
	uv run --directory backend arachne_fullstack server run --reload

run-prod:
	uv run --directory backend arachne_fullstack server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend arachne_fullstack server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend arachne_fullstack user create-admin

user-create:
	uv run --directory backend arachne_fullstack user create

user-list:
	uv run --directory backend arachne_fullstack user list

# === Celery ===
celery-worker:
	uv run --directory backend arachne_fullstack celery worker

celery-beat:
	uv run --directory backend arachne_fullstack celery beat

celery-flower:
	uv run --directory backend arachne_fullstack celery flower
	@echo ""
...
help:
	@echo ""
	@echo "arachne_fullstack - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
